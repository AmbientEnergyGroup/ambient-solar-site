...
export default function Projects() {
  ...
  const getUserProjects = () => {
    ...
    if (!isAdmin) {
      projectsData = projectsData.filter(project => project.userId === user.uid);
    }
  ...
  const calculateProjectMilestoneCommission = (project: Project) => {
    console.log("===== CALCULATING MILESTONE COMMISSION =====");
    console.log("Project:", project);
    console.log("System Size:", project.systemSize);
    console.log("Payment Amount:", project.paymentAmount);
    console.log("Selected Pay Type:", selectedPayType);
    
    if (!project.systemSize || !project.paymentAmount) {
      console.log("Missing systemSize or paymentAmount, returning 0");
      return 0;
    }
    ...
    if (isNaN(systemSize) || isNaN(project.paymentAmount)) {
      console.log("Invalid systemSize or paymentAmount, returning 0");
      return 0;
    }
    
    console.log("Parsed System Size:", systemSize);
    
    // Calculate base cost: kW × $3,500
    const baseCost = systemSize * 3500;
    console.log("Base Cost (kW × $3,500):", baseCost);
    ...
    if (project.eaBattery) {
      adders += 8000;
      console.log("E/A Battery added: $8,000");
    }
    
    // Backup Battery: $13,000
    if (project.backupBattery) {
      adders += 13000;
      console.log("Backup Battery added: $13,000");
    }
    
    // MPU: $3,500
    if (project.mpu) {
      adders += 3500;
      console.log("MPU added: $3,500");
    }
    
    // HTI: $2,500
    if (project.hti) {
      adders += 2500;
      console.log("HTI added: $2,500");
    }
    
    // Reroof: $15,000
    if (project.reroof) {
      adders += 15000;
      console.log("Reroof added: $15,000");
    }
    
    console.log("Total Adders:", adders);
    
    // Calculate total cost: Base cost + Adders
    const totalCost = baseCost + adders;
    console.log("Total Cost (Base + Adders):", totalCost);
    
    // Calculate commission: Contract Price - Total Cost
    const commissionAmount = project.paymentAmount - totalCost;
    console.log("Commission Amount (Contract - Total Cost):", commissionAmount);
    ...
    console.log("Final Milestone Commission:", milestoneCommission);
      ...
      {showPayTypeModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="theme-bg-tertiary p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <p className="theme-text-secondary mb-4">
              Current Pay Type: <span className="font-semibold theme-text-primary">{selectedPayType}</span>
            </p>
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="grid grid-cols-1 gap-2">
                ...
                >
                  ...
                  <div className="text-sm theme-text-secondary">24% of commission</div>
                </button>
                ...
                >
                  ...
                  <div className="text-sm theme-text-secondary">39% of commission</div>
                </button>
                ...
                >
                  ...
                  <div className="text-sm theme-text-secondary">50% of commission</div>
                </button>
              </div>
            </div>
                ...
                onClick={async () => {
                  try {
                    if (user && userData && user.uid) {
                      ...
                      // Update user data in localStorage with user-specific key
                      const updatedUserData = { ...userData, payType: selectedPayType };
                      localStorage.setItem(userDataKey, JSON.stringify(updatedUserData));
                      
                      // Also store pay type separately for easy access
                      localStorage.setItem(userPayTypeKey, selectedPayType);
                      
                      // Update local state
                      console.log('Pay type updated locally for user:', user.uid, selectedPayType);
                      
                      setShowPayTypeModal(false);
                      
                      // Show success message
                      alert(`Pay type successfully updated to ${selectedPayType}!`);
