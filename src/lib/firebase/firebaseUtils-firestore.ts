...
// Note: Project interface already exists above, extending it with new fields
// Additional fields for the new workflow:
...
export const createUserData = async (user: User, role: 'admin' | 'setter' | 'closer' | 'manager' | 'region_admin' | 'office_admin' | 'owner_admin' = 'setter', recruitedBy?: string) => {
  ...
  await setDoc(doc(getFirebaseDB(), 'users', user.uid), userData);
  return userData;
};
...
export const getUserData = async (userId: string): Promise<UserData | null> => {
  try {
    const db = getFirebaseDB();
    const docRef = doc(getFirebaseDB(), 'users', userId);
...
export const createProject = async (projectData: Omit<Project, 'id' | 'createdAt' | 'updatedAt'>) => {
  ...
  const docRef = await addDoc(collection(getFirebaseDB(), 'projects'), project);
...
export const updateProject = async (projectId: string, updates: Partial<Project>) => {
  try {
    const docRef = doc(getFirebaseDB(), 'projects', projectId);
    await updateDoc(docRef, {
      ...updates,
      updatedAt: new Date().toISOString()
    });
...
export const createCustomerSet = async (setData: Omit<CustomerSet, 'id' | 'createdAt' | 'updatedAt'>) => {
  ...
  const docRef = await addDoc(collection(getFirebaseDB(), 'customerSets'), customerSet);
...
export const getUserCustomerSets = async (userId: string): Promise<CustomerSet[]> => {
  try {
    const q = query(
      collection(getFirebaseDB(), 'customerSets'),
      where('userId', '==', userId),
      orderBy('createdAt', 'desc')
    );
...
export const updateCustomerSet = async (setId: string, updates: Partial<CustomerSet>) => {
  try {
    const docRef = doc(getFirebaseDB(), 'customerSets', setId);
    await updateDoc(docRef, {
      ...updates,
      updatedAt: new Date().toISOString()
    });
...
export const deleteCustomerSet = async (setId: string) => {
  try {
    await deleteDoc(doc(getFirebaseDB(), 'customerSets', setId));
  } catch (error) {
...
export const subscribeToUserData = (userId: string, callback: (data: UserData | null) => void) => {
  return onSnapshot(doc(getFirebaseDB(), 'users', userId), (doc) => {
    if (doc.exists()) {
...
export const getAllUsers = async (): Promise<UserData[]> => {
  try {
    const querySnapshot = await getDocs(collection(getFirebaseDB(), 'users'));
    return querySnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data()
    })) as UserData[];
...
export const updateDocument = (collectionName: string, id: string, data: any) =>
  updateDoc(doc(getFirebaseDB(), collectionName, id), data);
...
export const backupUserData = async (userId: string): Promise<void> => {
  try {
    ...
    if (userData) {
      ...
      await setDoc(doc(getFirebaseDB(), 'userBackups', `${userId}_${Date.now()}`), {
        ...userData,
...
export const backupAllData = async (): Promise<void> => {
  try {
    ...
    const projects = await getDocs(collection(getFirebaseDB(), 'projects'));
    const customerSets = await getDocs(collection(getFirebaseDB(), 'customerSets'));
    ...
    await setDoc(doc(getFirebaseDB(), 'systemBackups', `backup_${Date.now()}`), backupData);
...
export const restoreUserData = async (userId: string, backupId: string): Promise<void> => {
  try {
    const backupDoc = await getDoc(doc(getFirebaseDB(), 'userBackups', backupId));
    if (backupDoc.exists()) {
      const backupData = backupDoc.data();
      await setDoc(doc(getFirebaseDB(), 'users', userId), backupData);
...
export const getDataIntegrityCheck = async (): Promise<{
  ...
  try {
    ...
    // Get latest backup
    const backups = await getDocs(collection(getFirebaseDB(), 'systemBackups'));
...
export const updateProjectWithBackup = async (projectId: string, updates: Partial<Project>): Promise<void> => {
  try {
    ...
    const projectDoc = await getDoc(doc(getFirebaseDB(), 'projects', projectId));
...
export const migrateUserData = async (userId: string): Promise<void> => {
  try {
    ...
    if (userData) {
      ...
      await setDoc(doc(getFirebaseDB(), 'users', userId), migratedData);
...
export const createSet = async (setData: Omit<CustomerSet, 'id' | 'createdAt'>): Promise<string> => {
  try {
    const db = getFirebaseDB();
    const docRef = await addDoc(collection(getFirebaseDB(), 'sets'), {
      ...setData,
      createdAt: new Date().toISOString(),
    });
    console.log('Set created with ID:', docRef.id);
    return docRef.id;
  } catch (error) {
...
export const assignSetToCloser = async (
  ...
  try {
    const db = getFirebaseDB();
    const setRef = doc(getFirebaseDB(), 'sets', setId);
...
export const deleteSet = async (setId: string): Promise<void> => {
  try {
    const db = getFirebaseDB();
    await deleteDoc(doc(getFirebaseDB(), 'sets', setId));
...
// ===== PROJECTS FUNCTIONS =====
// Note: Project functions already exist above, these are additional ones for the new workflow

// Update project status (additional function for new workflow)
export const updateProjectStatusNew = async (projectId: string, status: Project['status']): Promise<void> => {
  try {
